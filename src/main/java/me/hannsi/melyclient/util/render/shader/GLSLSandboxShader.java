...
public class GLSLSandboxShader {
    ...
    public GLSLSandboxShader(ResourceLocation rl) throws IOException {
        ...
        glAttachShader(program, createShader(Minecraft.getMinecraft().getResourceManager().getResource(new ResourceLocation("mely/render/shader/passthrough.vsh")).getInputStream(), GL_VERTEX_SHADER));
        glAttachShader(program, createShader(Minecraft.getMinecraft().getResourceManager().getResource(rl).getInputStream(), GL_FRAGMENT_SHADER));
